...
//block 출력
	...
	for(i=selecty ; i<=selecty+1 ; i++)
	{
		for(j=selectx ; j<=selectx+1 ; j++)
		{
			...
			old2=(HBITMAP *)SelectObject(bitDC,maskbit);
			BitBlt(MemDC,pblock[i][j].y,pblock[i][j].x,50,50,bitDC,0,0,SRCAND);
			SelectObject(bitDC,old2);
			old2=(HBITMAP *)SelectObject(bitDC,blockbit[block[i][j]]);
			cho[nc++]=Imakepoint(j,i);
			BitBlt(MemDC,pblock[i][j].y,pblock[i][j].x,50,50,bitDC,0,0,SRCPAINT);
			SelectObject(bitDC,old2);
		if(selectx%2==0)
			i++;
		}
...
void rotate()
{
	...
	block[cho[1].y][cho[1].x]=block[cho[2].y][cho[2].x];
	block[cho[2].y][cho[2].x]=block[cho[3].y][cho[3].x];
	block[cho[3].y][cho[3].x]=temp;
	Point temp2;
	temp2=pblock[cho[1].y][cho[1].x];
	pblock[cho[1].y][cho[1].x]=pblock[cho[2].y][cho[2].x];
	pblock[cho[2].y][cho[2].x]=pblock[cho[3].y][cho[3].x];
