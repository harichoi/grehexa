...
HBITMAP levelbit[100],levelmaskbit, copyright,specialbit,specialmaskbit;
const int nblock=5;
Block block[100][100], tblock[100][100];
int dieblock[100][100];
Point wblock[100][100];
				   ...
				   int nShow)
{
	...
	levelbit[2]=LoadBitmap(hInst,MAKEINTRESOURCE(IDB_LEVEL2));
	...
	specialbit=LoadBitmap(hInst,MAKEINTRESOURCE(IDB_SPECIAL));
	specialmaskbit=LoadBitmap(hInst,MAKEINTRESOURCE(IDB_SPECIALMASK));
...
LRESULT CALLBACK WndProc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
		...
		for(i=1 ; i<=HEIGHT+1 ; i++)
			for(j=1 ; j<=WIDTH ; j++){
				if(i==HEIGHT+1 && j%2==1)
					continue;
				if(block[i][j].bomb>0)
				if(rand()%block[i][j].bomb*block[i][j].bomb==0)
					block[i][j].bombcolor=RGB(rand()%256,rand()%256,rand()%256);
				if(nr==0){
					if(hide==0){
					block[i][j].point.y+=(wblock[i][j].y-block[i][j].point.y)/3;
					block[i][j].point.x+=(wblock[i][j].x-block[i][j].point.x)/3;
					}
				}
				else
				{
					...
					for(k=1 ; k<=3 ; k++)
						if(cho[k].y==i && cho[k].x==j)
							hh=1;
					if(hh==1){
						if(wblock[i][j].x>block[i][j].point.x)
							block[i][j].point.x+=3.5;
						if(wblock[i][j].y>block[i][j].point.y)
							block[i][j].point.y+=3.5;
						if(wblock[i][j].x<block[i][j].point.x)
							block[i][j].point.x-=3.5;
						if(wblock[i][j].y<block[i][j].point.y)
							block[i][j].point.y-=3.5;
	...
	for(i=2 ; i<=HEIGHT ; i++)
		for(j=2 ; j<=WIDTH-1 ; j++)
		{
			if(j%2==1 && i==HEIGHT)
				continue;
			if(i==HEIGHT+1 && j%2==0 || block[i][j].block!=6)
				continue;
		ax=j*25+DEFX-7;
		ay=i*31+DEFY+12;
...
//block 출력
	...
	for(i=1 ; i<=HEIGHT+1 ; i++)
	{
		for(j=1 ; j<=WIDTH ; j++)
		{
			...
			BitBlt(MemDC,block[i][j].point.x,block[i][j].point.y,50,50,bitDC,0,0,SRCAND);
			SelectObject(bitDC,old2);
			old2=(HBITMAP *)SelectObject(bitDC,blockbit[block[i][j].block]);
			BitBlt(MemDC,block[i][j].point.x,block[i][j].point.y,50,50,bitDC,0,0,SRCPAINT);
			SelectObject(bitDC,old2);
			if(block[i][j].bomb>0)
			{
				sprintf(str,"%d",block[i][j].bomb);
				SetTextColor(MemDC,block[i][j].bombcolor);
				TextOut(MemDC,block[i][j].point.x+DEFX-15,block[i][j].point.y+DEFY+7,str,strlen(str));
			}
			if(block[i][j].special==1)
			{
			old2=(HBITMAP *)SelectObject(bitDC,specialmaskbit);
			BitBlt(MemDC,block[i][j].point.x,block[i][j].point.y,50,50,bitDC,0,0,SRCAND);
			SelectObject(bitDC,old2);
			old2=(HBITMAP *)SelectObject(bitDC,specialbit);
			BitBlt(MemDC,block[i][j].point.x,block[i][j].point.y,50,50,bitDC,0,0,SRCPAINT);
			SelectObject(bitDC,old2);
			}
		}
	...
	{	
		for(i=1 ; i<=HEIGHT+1 ; i++)
		{
			...
			SelectObject(bitDC,old2);
			old2=(HBITMAP *)SelectObject(bitDC,blockbit[tblock[i][j].block]);
	...
	SelectObject(bitDC,old2);
	old2=(HBITMAP *)SelectObject(bitDC,levelbit[level]);
...
void process()
{
	...
	Block reblock[100][100];
	
	for(i=0 ; i<=HEIGHT+1 ; i++)
		for(j=0 ; j<=WIDTH ; j++)
			dieblock[i][j]=0,reblock[i][j].clear();
	for(i=0 ; i<=HEIGHT+1 ; i++)
		for(j=1 ; j<=WIDTH ; j++)
	...
	for(i=1 ; i<=HEIGHT ; i++)
		for(j=1 ; j<=WIDTH ; j++)
		{
			...
			if(j%2==1)
			{
				if(block[i][j].block==block[i][j+1].block && block[i][j].block==block[i][j+2].block)
					if(block[i][j].block==block[i+1][j].block && block[i][j].block==block[i+1][j+2].block && block[i][j].block==block[i+2][j+1].block)
					{
						reblock[i+1][j+1].block=6;
						reblock[i][j].block=0;
						reblock[i][j+1].block=0;
						reblock[i][j+2].block=0;
						reblock[i+1][j].block=0;
						reblock[i+1][j+2].block=0;
						reblock[i+2][j+1].block=0;
						reblock[i+1][j+1].bomb=0;
						score+=2000;
						hide=1;
					}
			}
			...
			{
				if(block[i][j].block==block[i-1][j+1].block && block[i][j].block==block[i][j+2].block)
					if(block[i][j].block==block[i+1][j].block && block[i][j].block==block[i+1][j+1].block && block[i][j].block==block[i+1][j+2].block)
					{
						reblock[i][j+1].block=6;
						reblock[i][j].block=0;
						reblock[i-1][j+1].block=0;
						reblock[i][j+2].block=0;
						reblock[i+1][j].block=0;
						reblock[i+1][j+1].block=0;
						reblock[i+1][j+2].block=0;
						reblock[i][j+1].bomb=0;
						score+=2000;
						hide=1;
					}
			}
			...
			if(j%2==1)
			{
				...
				if(block[i][j].special==1 && block[i+1][j].special==1 && block[i+1][j+1].special==1)
				{
					reblock[i][j].block=0;
					reblock[i+1][j].block=0;
					reblock[i+1][j+1].block=0;
				...
				if(block[i][j].special==1 && block[i][j+1].special==1 && block[i+1][j+1].special==1)
				{
					reblock[i][j].block=0;
					reblock[i][j+1].block=0;
					reblock[i+1][j+1].block=0;
			...
			if(j%2==0)
			{
				if(block[i][j].special==1 && block[i+1][j].special==1 && block[i][j+1].special==1)
				{
					reblock[i][j].block=0;
					reblock[i+1][j].block=0;
					reblock[i][j+1].block=0;
				...
				if(block[i][j].special==1 && block[i-1][j+1].special==1 && block[i][j+1].special==1)
				{
					reblock[i][j].block=0;
					reblock[i-1][j+1].block=0;
					reblock[i][j+1].block=0;
			...
			if(j%2==1)
			{
				if(block[i][j].block==block[i+1][j].block && block[i+1][j+1].block==block[i][j].block)
				{
					reblock[i][j].block=0;
					reblock[i+1][j].block=0;
					reblock[i+1][j+1].block=0;
				...
				if(block[i][j].block==block[i][j+1].block && block[i+1][j+1].block==block[i][j].block)
				{
					reblock[i][j].block=0;
					reblock[i][j+1].block=0;
					reblock[i+1][j+1].block=0;
			...
			if(j%2==0)
			{
				if(block[i][j].block==block[i+1][j].block && block[i][j+1].block==block[i][j].block)
				{
					reblock[i][j].block=0;
					reblock[i+1][j].block=0;
					reblock[i][j+1].block=0;
				...
				if(block[i][j].block==block[i-1][j+1].block && block[i][j+1].block==block[i][j].block)
				{
					reblock[i][j].block=0;
					reblock[i-1][j+1].block=0;
					reblock[i][j+1].block=0;
	...
	for(i=1 ; i<=HEIGHT+1 ; i++)
		for(j=1 ; j<=WIDTH ; j++){
			if(i==HEIGHT+1 && j%2==1)
				continue;
			if(reblock[i][j].block==0)
			{
				nnnnn++;
				dieblock[i][j]=1;
				for(k=i ; k>=1 ; k--)
					reblock[k][j]=reblock[k-1][j];
			}
		}
		if(nnnnn>2)
		score+=(nnnnn-2)*5;
	for(i=1 ; i<=HEIGHT+1 ; i++)
		for(j=1 ; j<=WIDTH ; j++){
			if(i==HEIGHT+1 && j%2==1)
				continue;
			if(reblock[i][j].block==0){
				reblock[i][j].block=rand()%5+1;
				reblock[i][j].bomb=0;
				if(rand()%50==0 && level>=3)
					reblock[i][j].bomb=9;
				if(rand()%6==0 && level>=2)
					reblock[i][j].special=1;
			}
		}
	for(i=1 ; i<=HEIGHT+1 ; i++)
		for(j=1 ; j<=WIDTH ; j++)
...
void calcul()
{
	...
	for(i=0 ; i<=HEIGHT+1 ; i++)
		for(j=1 ; j<=WIDTH ; j++)
			block[i][j].point=makepoint(j*25,-50);
		hide=0;
}
...
int finish()
{
	...
	for(i=1 ; i<=HEIGHT+1 ; i++)
	{
		for(j=1 ; j<=WIDTH ; j++)
		{
			if(i==HEIGHT+1 && j%2==1)
				continue;
			ttt+=hypot(wblock[i][j].y-block[i][j].point.y,wblock[i][j].x-block[i][j].point.x);
		}
	}
	if(ttt<15 && nr==0)
		return 1;
